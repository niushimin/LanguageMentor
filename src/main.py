...
agents = {
    ...
    "leave_request": ScenarioAgent("leave_request"),  # 工作请假场景代理
...
def handle_conversation(user_input, chat_history):
    bot_message = conversation_agent.chat_with_history(
        user_input
    )  # 获取聊天机器人的回复
...
with gr.Blocks(title="LanguageMentor 英语私教") as language_mentor_app:
    with gr.Tab("场景训练"):  # 场景训练标签
        ...
        def start_new_scenario_chatbot(scenario):
            initial_ai_message = agents[
                scenario
            ].start_new_session()  # 启动新会话并获取初始AI消息
        ...
        scenario_radio.change(
            fn=lambda s: (
                get_scenario_intro(s),
                start_new_scenario_chatbot(s),
            ),  # 更新场景介绍和聊天机器人
